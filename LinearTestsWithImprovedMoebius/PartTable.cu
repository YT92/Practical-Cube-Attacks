  ...
  __host__ void construc_truth_table_dynamic_CutM_32(u32 cube[], u32 dim, u32 roundnum,u32 **table,u32 rownum, u32 columnnum,u32 loadkey[10],u32 lk)
{
	...
	u32 bolcknum=128,threadnum=512;//00.06
...
u32 linearity_test_dynamicV6(u32 cube[], u32 dim, u32 roundnum,char filename[])
{
	...
	u32 numrandomkey = 140,tttt;  // 两两一组
	u32 randomkey[256*2][10],sfkey[256*2][10]={0},sfkey2[256][10]={0},tempkey[256][10]={0};//第一组密钥
	u32 randomkeypro[256*2][10]={0},sfkeypro[256*2][10]={0},sfkeypro2[256][10]={0};//第二组密
	u8 breakflag=0;
	clock_t t1,t2;
	//u32 **ANF;//还是需要排除常数项
	FILE *fp2,*Sdim;
...
int main()
{
	...
	//u32 cube[40]={0,1,2,4,6,8,10,11,13,15,17,19,21,23,25,26,27,28,29,32,34,36,38,39,41,42,43,45,47,48,50,52,53,57,59,69,71,75,76,79};
	//u32 cube[43]={77,73,71,70,60,56,55,50,47,46,45,37,32,30,28,27,26,22,18,15,6,2};//710
	//u32 cube[43]={1,3,4,7,9,10,12,16,19,21,25,27,29,30,32,34,35,37,40,47,50,51,60,61,64,67,72,73,79};
	//u32 cube[43] = { 2,4,6,10,11,12,15,17,19,21,23,25,29,34,36,41,0,70,8,16,79,27,45,26,28,31,77,38,47,13,1,62,49,64,40,39,43,50,58,74 };
	u32 cube[43] = { 4,6,10,11,15,17,19,21,25,29,32,34,36,39,41,43,50,2,69,79,8,27,0,1,28,71,13,45,23,26,38,76,47,52,57,42,48,58,59,63 };
